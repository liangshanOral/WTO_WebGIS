...
def echarts_data(request):
    ...
    # 将传递的字符串参数拆分成列表
    selected_countries = request.GET.get('country', '').split(',')
    selected_products = request.GET.get('product', '').split(',')

    # 处理产品参数，确保每个产品名称被引号包裹
    selected_products = [f'"{product.strip()}"' for product in selected_products]

    print(selected_countries, selected_products)
