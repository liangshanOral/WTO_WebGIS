...
# 现在可以导入 Django 模型
from wto.models import TradeYearData_I_Individual, Country, ProductSector

# 读取 Excel 文件
df = pd.read_excel('F:/zyy_wto/data/WtoData_import_Individual.xlsx')
...
for index, row in df.iterrows():
    ...
    for year_col in year_columns:
        ...
        TradeYearData_I_Individual.objects.create(
            reporting_country=reporting_country,
            product_sector=product_sector,
            partner_country=partner_country,
            year=year,
            import_value_y=export_value_y,
