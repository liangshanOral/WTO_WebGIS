...
function updateflow(data) {
    ...
    // 在合适的地方注册你的地图数据，这里使用一个简单的示例
    echarts.registerMap('customMap', {
        type: 'FeatureCollection',
        features: [
            { type: 'Feature', properties: {}, geometry: { type: 'Point', coordinates: [0, 0] } }, // 一个简单的中心点，可以根据实际情况更改
        ]
    });
    ...
    var option = {
        ...
        series: [{
            ...
            data: edges.map(edge => {
                const sourceCoords = nodes[edge.source];
                const targetCoords = nodes[edge.target];
                ...
                if (sourceCoords && targetCoords) {
                    return {
                        ...
                        toName: edge.target,
                        coords: [
                            { name: edge.source, value: [sourceCoords.longitude, sourceCoords.latitude, edge.value] },
                            { name: edge.target, value: [targetCoords.longitude, targetCoords.latitude, edge.value] }
                        ],
                        value: edge.value
                    };
                } else {
                    // 如果坐标不存在，可以根据实际情况进行处理，例如忽略这条边或给出默认值
                    return null;
                }
