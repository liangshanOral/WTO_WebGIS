...
def echarts_flow_data(request):
    ...
    for entry in queryset:
        ...
        # 检测坐标是否存在
        if all(value is not None for value in reporting_country_node.values()):
            nodes[reporting_country] = reporting_country_node

        if all(value is not None for value in partner_country_node.values()):
            nodes[partner_country] = partner_country_node
        # 添加边
        edges.append({
