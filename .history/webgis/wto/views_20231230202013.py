...
def echarts_flow_data(request):
    ...
    for time in selected_time:
        for product in selected_product:
            query |= Q(year__name=time, product_sector__name=product)
    
    # 查询数据库
    queryset = CommercialData_E.objects.filter(query)
    print(queryset)
    # 将 CommercialData_E 对象转为字典
    data = [{'year': entry.year, 'country': entry.reporting_country.name, 'product': entry.product_sector.name, 'export_value_y': entry.export_value} for entry in queryset]
    return JsonResponse({'data': data})
