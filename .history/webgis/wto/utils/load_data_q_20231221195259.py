...
# 现在可以导入 Django 模型
from wto.models import TradeQuarterData_E, Country, ProductSector

# 读取 Excel 文件
df = pd.read_excel('F:/zyy_wto/data/WtoData_export_q.xlsx')
...
for index, row in df.iterrows():
    ...
    for year_col in year_columns:
        if row['Partner Economy']=='World':
            ...
            TradeQuarterData_E.objects.create(
                reporting_country=reporting_country,
                year=year,
                quarter=quarter,
                export_value_qpyt=export_value_y,
