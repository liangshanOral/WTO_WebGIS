...
def echarts_flow_data(request):
    ...
    for time in selected_time:
        for product in selected_product:
            query |= Q(year=time, product_sector__name=product)
    ...
    for entry in queryset:
        ...
        # 添加节点
        nodes.add(reporting_country)
        nodes.add(partner_country)
    ...
    # 将 CommercialData_E 对象转为字典
    data = [{'year': entry.year, 'country': entry.reporting_country.name, 'product': entry.product_sector.name, 'export_value': entry.export_value} for entry in queryset]
    return JsonResponse({'data': data})
